...
class HomeActivity : AppCompatActivity(), CoroutineScope {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val gamesStore = SessionStorage.getGames()
        if(gamesStore.isEmpty()) {
            getTopGames()
        }
        else {
            assignGameUI(gamesStore)
        }
    ...
    private fun getTopGames() {
        ...
        launch {
            ...
            if (data.has("error")) {
                ...
                assignGameUI(games)
                SessionStorage.setGames(games)
            }
        }
    }

    private fun assignGameUI(games: List<Game>) {
